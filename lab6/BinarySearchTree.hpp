...
public:
  ...
  //LAB 6
  void widthTraversal(std::ostream& out);
  std::vector<T> prefixTraversal(std::ostream& out, bool mode);
  bool sameTrees(BinarySearchTree<T> &tree);
...
template<class T>
void BinarySearchTree<T>::widthTraversal(std::ostream& out)
{
  QueueList<Node*> queue;
  queue.insertQueue(root_);
  while (!(queue.isEmpty()))
  {
    ...
    if (temp->left_ != nullptr)
    {
      queue.insertQueue(temp->left_);
    }
    if (temp->right_ != nullptr)
    {
      queue.insertQueue(temp->right_);
    }
  }
}

template<class T>
std::vector<T> BinarySearchTree<T>::prefixTraversal(std::ostream& out, bool mode)
{
  std::vector<T> arr;
...
template<class T>
bool BinarySearchTree<T>::sameTrees(BinarySearchTree<T> &tree)
{
  std::vector<T> arrOne = prefixTraversal(std::cout, false);
  std::vector<T> arrTwo = tree.prefixTraversal(std::cout, false);
